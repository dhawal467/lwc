...
export async function GET() {
  ...
  const { count: activeOrders, error: _activeError } = await supabase
    .from('orders')
  ...
  const { count: overdueOrders, error: _overdueError } = await supabase
    .from('orders')
    .select('*', { count: 'exact', head: true })
  ...
  const { data: outstandingData, error: _outstandingError } = await supabase
    .from('orders')
